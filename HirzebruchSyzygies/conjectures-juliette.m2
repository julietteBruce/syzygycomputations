...
apply(dataRange,D->(
	H = schurBetti(0,D#0,D#1);
	apply(keys H,k->(
		if isSubset({(k#0-1,k#1+1)},keys H) == true then (
	    	    H1 = deleteMulti(H);
	    	    L1 = toList( (set H1#k)*(set H1#(k#0-1,k#1+1)));
		    if L1 != {} then M#{D} = L1; 
	    	    );
		));
	))

L1 = set flatten keys M;
L2 = set dataRange;
#(L2-L1)
#L1
#L2
...
#flatten delete({},values schurBetti(0,{0,8},{2,10}))
...
---- Finds the examples where redundant entries occur in mutlple rows.
toExclude = delete(,apply(dataRange,D->(
	H = schurBetti(0,D#0,D#1);
	M = new MutableHashTable;
	apply(keys H,k->(
		if isSubset({(k#0-1,k#1+1)},keys H) == true then (
	    	    H1 = deleteMulti(H);
	    	    L1 = toList( (set H1#k)*(set H1#(k#0-1,k#1+1)));
		    if (#(H1#k))!=0 then M#k = #L1/(#(H1#k));
		    --if (#(H1#k))==0 then M#k = 2;	    
	    	    );
		--if isSubset({(k#0-1,k#1+1)},keys H) != true then (
		--    M#k = 2;
		--    )
		));
	L2 = unique delete(,apply(keys M,k->(
		    if M#k != 0 then k#1
		    ))
	    );
	if #L2 >1 then D
...
dataRange2 = toList( (set dataRange) - (set toExclude));

orderedData = apply(dataRange2,D->(
	M = max flatten D;
	{M}|D
	));

L5 = flatten apply(dataRange2,D->(
	H = schurBetti(0,D#0,D#1);
	M = new MutableHashTable;
	apply(keys H,k->(
		if isSubset({(k#0-1,k#1+1)},keys H) == true then (
	...
	if L2 != {} then rowQ = L2#0;
	if L2 == {} then rowQ = 0;
	augD = {max flatten D}|D;
       	pos = position(orderedData,x->x==augD);
	pd = max apply(keys H,k->k#0);
	print pos;
	apply(toList(0..pd),p->(
		k = (p,rowQ);
		if isSubset({(p,rowQ)},keys M)== true then ({p/pd,pos,M#k})
		else (
		     {p/pd,pos,0}
        	    )
		))
 ...
 g = openOut ("heatPlots.m2");
 g << toExternalString L5;
 close g
# 
dataRange2 = {{{2,4},{3,5}}}
...
unique delete(,apply(keys M, k->(
	if M#k != 0 then k#1
	)))	

H = schurBetti(0,{2,0},{3,4});
totalBettiTally(0,{2,4},{3,5})

pd = max apply(keys H,k->k#0)
apply({0,1,2},q->(
	apply(toList(0..pd),p->(
		k = (p,q);
		if isSubset({k},keys M) == true then (
		    M#k
		    )
		else (
		    2
...
apply(keys H,k->(
	if isSubset({(k#0-1,k#1+1)},keys H) == true then (
	    H1 = deleteMulti(H);
	    M#k = toList( (set H1#k)*(set H1#(k#0-1,k#1+1)))
	    )
	))
...
netList (dominantWeightsBetti(0,{0,0},{2,9}))#(18,2)
