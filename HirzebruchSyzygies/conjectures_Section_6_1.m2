...
--------------------------------------------------------------------
--------------------------------------------------------------------
secondLastSchurConjectureQ1 = method();
secondLastSchurConjectureQ1 (ZZ,List,List,ZZ) := (a,B,D,q) ->(
    if a != 0 or B != {0,0} or q != 1 then true
    else (
...
testConjecture(0,1,secondLastSchurConjectureQ1,ShowFails=>true)
...
-----
----- DESCRIPTION: This function tests whether our conjecture concerning 
----- the Schur functor decomposition for the last entry in the q=2
----- row is true for the input (a,B,D,q).
-----
----- CAVEAT: This conjecture is currently only made for a=0, 
----- B={0,0}, and q=2. If any other case is tested this function will
----- just return true. 
-----
----- NOTES: When D={1,*} there are no entries in the q=2 row. 
--------------------------------------------------------------------
--------------------------------------------------------------------
lastSchurConjectureQ2 = method();
lastSchurConjectureQ2 (ZZ,List,List,ZZ) := (a,B,D,q) ->(
    if a != 0 or B != {0,0} or q != 2 or D#0 == 1 then true
    else (
        A1 = binomial((D#0)+1,2)*binomial((D#1)+1,1)-1;
	A2 = binomial((D#0)+1,2)*binomial((D#1)+1,1)-(D#0)+1; 
	A3 = binomial((D#0)+1,1)*binomial((D#1)+1,2)-1;
	A4 = binomial((D#0)+1,1)*binomial((D#1)+1,2)-(D#1)+1;
	conj := {A1,A2,A3,A4};
	{(conj,1)} == lastSchurEntry(a,B,D,q)
	)
    )
...
-----
----- DESCRIPTION: This function tests whether our conjecture concerning 
----- the Schur functor decomposition for the second to last entry in the q=2
----- row is true for the input (a,B,D,q) where D={2,d}.
-----
----- CAVEAT: This conjecture is currently only made for a=0, 
----- B={0,0}, and q=2. If any other case is tested this function will
----- just return true. 
...
--------------------------------------------------------------------
--------------------------------------------------------------------
secondLastSchurConjectureQ2D2 = method();
secondLastSchurConjectureQ2D2 (ZZ,List,List,ZZ) := (a,B,D,q) ->(
    if a != 0 or B != {0,0} or q != 2 or D#0 != 2 then true
    else (
	ans := secondLastSchurEntry(a,B,D,q);
	if ans == {-infinity} then return true;
	if ans != {-infinity} then (
	    conj = apply((D#1)-2,i->(
		    A1 := 3*(D#1)+2;
		    A2 := 3*(D#1);
		    A3 := .5*(3*(D#1)^2+3*(D#1))-2-i;
		    A4 := .5*(3*(D#1)^2-(D#1))+2+i;
		    ({A1,A2,A3,A4},1)
		    ));
	    return conj == ans;
	    )
...
testConjecture(0,1,secondLastSchurConjectureQ2D2,ShowFails=>true)
...
----- DESCRIPTION: This function tests whether our conjecture concerning 
----- the Schur functor decomposition for the second to last entry in the q=2
----- row is true for the input (a,B,D,q) for D={3,d}.
-----
----- CAVEAT: This conjecture is currently only made for a=0, 
----- B={0,0}, and q=2. If any other case is tested this function will
----- just return true. 
...
--------------------------------------------------------------------
--------------------------------------------------------------------
secondLastSchurConjectureQ2D3 = method();
secondLastSchurConjectureQ2D3 (ZZ,List,List,ZZ) := (a,B,D,q) ->(
    if a != 0 or B != {0,0} or q != 2 or D#0 != 3 then true
    else (
	ans := secondLastSchurEntry(a,B,D,q);
	if ans == {-infinity} then return true;
	if ans != {-infinity} then (
	    conj1 = apply((D#1)-2,i->(
		    A1 := 6*(D#1)+5;
		    A2 := 6*(D#1)+1;
		    A3 := 2*(D#1)^2+2*(D#1)-2-i;
		    A4 := 2*(D#1)^2+2*(D#1)-2*(D#1)+3+i;
		    ({A1,A2,A3,A4},1)
		    ));
	    conj2 = apply((D#1)-1,i->(
		    A1 := 6*(D#1)+4;
...
testConjecture(0,1,secondLastSchurConjectureQ2D3,ShowFails=>true)
...
--------------------------------------------------------------------
--------------------------------------------------------------------
secondLastSchurQuestionQ2 = method();
secondLastSchurQuestionQ2 (ZZ,List,List,ZZ) := (a,B,D,q) ->(
    if a != 0 or B != {0,0} or q != 2 or D#0 == 1 then true
    else (
	ans := secondLastSchurEntry(a,B,D,q);
	if ans == {-infinity} then return true;
	if ans != {-infinity} then (
	    conj = apply((D#1)-2,i->(
		    A1 := binomial((D#0)+1,2)*binomial((D#1)+1,1)-1;
		    A2 := binomial((D#0)+1,2)*binomial((D#1)+1,1)-2*(D#0)+1; 
		    A3 := binomial((D#0)+1,1)*binomial((D#1)+1,2)-2-i;
		    A4 := binomial((D#0)+1,1)*binomial((D#1)+1,2)-2*(D#1)+2+i; 
		    ({A1,A2,A3,A4},1)
		    ));
	    return isSubset(conj,ans);
	    )
    )
)

testConjecture(0,1,secondLastSchurQuestionQ2,ShowFails=>true)
